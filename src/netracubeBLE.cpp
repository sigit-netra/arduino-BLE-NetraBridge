...
void netracubeBLE::task_scheduler () {
    ...
    const unsigned long interval  = 4 * 60 * 60 * 1000; // 4 jam
    const unsigned long delay_sos = 10 * 60 * 1000;     // 10 menit

    // const unsigned long interval  = 3 * 60 * 1000; // 4 jam
    // const unsigned long delay_sos = 1 * 60 * 1000; // 10 menit
    auto _status                  = deviceStatus::GetInstance ();
    ...
    while (1) {
        if (_status->get_ble_status () == 1) {
            ...
            if (currentMillis - previousMillis >= interval) {
                previousMillis = currentMillis; // Simpan waktu saat ini sebagai referensi berikutnya
